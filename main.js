...
function showTaskContextMenu(store, task, event) {
...
var BelkiSettingTab = class extends import_obsidian2.PluginSettingTab {
    ...
    return [
      {
        ...
        control: {
          ...
          validate: (value) => this.validateTasksFilePath(value)
        }
      },
      {
  ...
  validateTasksFilePath(value) {
    const path = (0, import_obsidian2.normalizePath)((value || "").trim() || DEFAULT_SETTINGS.tasksFilePath);
  ...
  buildSectorList() {
    return {
      ...
      items: this.plugin.settings.sectors.map((sector, index) => ({
        ...
        render: (setting) => this.renderSectorRow(setting, sector, index)
      }))
    };
  }
  renderSectorRow(setting, sector, index) {
  ...
  buildAppearancePage() {
    ...
    const projectItems = projects.length === 0 ? [{
      ...
      render: (setting) => this.renderProjectColorRow(setting, project)
    }));
    const labelItems = [
      {
        name: "Add label",
        desc: "Create a label without assigning it to a task yet.",
        render: (setting) => this.renderAddLabelRow(setting)
      },
      ...labels.length === 0 ? [{
        name: "",
        ...
        render: (setting) => this.renderLabelColorRow(setting, label)
      }))
    ];
    return {
      type: "page",
      ...
      items: [
        ...
        },
        {
          ...
          items: iconRows.map(([name, key]) => ({
            name,
            desc: 'Lucide icon name (see lucide.dev/icons), e.g. "calendar-check".',
            control: {
              type: "text",
              key: `icons.${key}`,
              validate: (value) => LUCIDE_ICON_NAME_PATTERN.test(value.trim()) ? void 0 : "Not a valid Lucide icon name (lowercase words separated by hyphens)."
            }
          }))
        },
  ...
  renderLabelColorRow(setting, label) {
