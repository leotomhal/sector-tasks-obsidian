...
- **Today sidebar (since v0.22.0):** the command **"Open Today sidebar"** opens a compact panel in Obsidian's right sidebar listing overdue and due-today tasks (archived sectors excluded). The circle completes a task (priority-colored), clicking a row opens it in the Tasks edit modal, and the `+` button quick-adds to the Inbox. The panel updates live with every change to the tasks file.
