...
- **Sector rollover (since v0.20.0):** each sector (except the first) has a "Rollover" setting: *On new week*, *On new month*, or *No rollover*. The command **"Roll over sectors (new week/month)"** moves all tasks of a rollover-enabled sector to the sector directly above it — but only once per period: weekly rollovers fire only when a new ISO week (Monday) has started since the last run, monthly ones only in a new calendar month. Re-running the command mid-week or mid-month does nothing, so a July run never pulls next month's tasks in. Defaults: `#02next-week` → `#01this-week` (weekly), `#04next-month` → `#03this-month` (monthly).
