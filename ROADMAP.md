...
- **Source-based build (fixes Obsidian's security-scan disclosures)** —
  ...
  source and attest the output.
  - Phase 1 (done): build tooling (`package.json` devDependencies,
    `tsconfig.json`, `esbuild.config.mjs`, `package-lock.json`).
  - Phase 2 (done): reconstructed `src/` (14 modules), `main.js` is now
    generated by `npm run build` (`tsc -noEmit` + esbuild) instead of
